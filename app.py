...
with tab1:
  ...
  def render_ev_catalog(data_df):
    if not data_df.empty:
      ...
      page_df = data_df.iloc[start_idx:end_idx].reset_index(drop=True)
            ...
            if is_selected:
              ...
              if st.button(
                ...
                if len(st.session_state["selected_compare"]) >= 4:
                  st.toast(
                      "⚠️ အများဆုံး ၄ စီးအထိသာ ယှဉ်ပြိုင်နိုင်ပါသည်။",
                      icon="⚠️",
                  )
                else:
      ...
      if st.session_state["selected_compare"]:
        ...
        with st.container(border=True):
          ...
          with d_col1:
            st.markdown(
                "**📌 ရွေးချယ်ထားသော ကားများ"
                f" ({len(st.session_state['selected_compare'])}/4):**"
            )
            badges = [
